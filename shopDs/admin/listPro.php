<?php
    ...
    //得到所有的商品
    $sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from ds_pro as p join ds_cate c on p.cId=c.id {$where}";
    ...
    $sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from ds_pro as p join ds_cate c on p.cId=c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
    $rows=fetchAll($sql);
    //print_r($rows);
    if (!$rows){
        alertMes("sorry,没有商品信息，请添加！", "addPro.php");
        exit;
    }
?>
<!doctype html>
        ...
        <table class="table">
            ...
            <?php foreach ($rows as $row):?>
                <tr>
                    ...
                    <td>
                    	<input type="checkbox" id="<?php echo $row['id'];?>" class="check" value=<?php echo $row['id'];?>>
                    ...
                    <td align="center">
                        ...
                        <div id="showDetail<?php echo $row['id'];?>" style="display:none;">
                        	<table class="table">
                        		...
                        		<tr>
                        			<td width="20%" align="right">电商网价格</td>
                        		...
                        		<tr>
                        			...
                        			<td>
										<?php
										  $images=getAllImgByProId($row['id']);
										  foreach ($images as $img):
										?>
										<img style='width:100px;height:100px;' src='uploads/<?php echo $img['albumPath'];?>' alt='img'/> &nbsp;&nbsp;
										<?php endforeach;?>
                        			</td>
                        		</tr>
                        		<tr>
